...
function createResearchTools(sessionId: string | null) {
  ...
  return {
    ...
    fetch_tensorflow_datasets: tool({
      description:
        'Search the TensorFlow Datasets catalogue to find the best public dataset for the current task, ' +
        'when the user has NOT uploaded their own data. ' +
        'Returns the recommended dataset_name and split info so it can be used directly in the training script via tfds.load(). ' +
        'The dataset is NOT downloaded on the server — instead, write tfds.load(dataset_name, ...) into the training script ' +
        'so the dataset downloads automatically when the user runs the notebook on their own machine. ' +
        'If no suitable dataset exists (available=false), fall back to synthetic data generation with tf.random.normal().',
      inputSchema: z.object({
        task_description: z.string().describe('Description of the ML task, e.g. "image classification of handwritten digits"'),
      }),
      execute: async (input: { task_description: string }) => {
        if (!sessionId) return { error: 'No active session' }
        try {
          ...
          return {
            ...
            instruction:
              'Review the catalog. If a suitable dataset exists, pick the best dataset_name (e.g. "mnist", "cifar10", "imdb_reviews"). ' +
              'Tell the user which dataset you selected and why. ' +
              'Then write tfds.load(dataset_name, split="train", as_supervised=True) directly into the training script — do NOT try to download it on the server. ' +
              'If no suitable dataset exists, set available=false and use tf.random.normal() synthetic data instead.',
          }
        } catch (e) {
...
In DESCRIPTION MODE, ALWAYS call fetch_tensorflow_datasets to find a suitable public dataset BEFORE falling back to synthetic data.
TensorFlow Datasets (tfds) provides hundreds of public datasets (MNIST, CIFAR-10, ImageNet subsets, IMDB, LibriSpeech, etc.).
If a suitable dataset is found: write tfds.load(dataset_name, ...) directly into the training script — the dataset downloads on the user's machine when they run the notebook. Tell the user which dataset will be used and why.
Only use synthetic tf.random.normal() data if no suitable tfds dataset exists.
</role>
...
- CRITICAL — For CNN / image models with no uploaded dataset: FIRST call fetch_tensorflow_datasets to identify a real dataset (e.g. mnist, cifar10, cifar100, oxford_flowers102, stanford_dogs). If one is found, write tfds.load(dataset_name, split="train", as_supervised=True) directly in the script — do NOT use dataset.csv, do NOT download on the server. The dataset downloads on the user's machine when they run the notebook. Only if no suitable dataset exists, generate synthetic image tensors with tf.random.normal(shape=(N, H, W, C)). NEVER reference a file path that does not exist.
...
3. USER DESCRIBES A VISION, VIDEO, TRANSFORMER, OR CUSTOM ARCHITECTURE TASK WITH NO DATASET?
   ...
   STEP 0 — TF Datasets check (ALWAYS do this first for vision/NLP/audio/video tasks):
     a. Call fetch_tensorflow_datasets(task_description="<user task>") to search for a suitable dataset.
     b. If a suitable dataset is found: tell the user which dataset_name will be used and why. The script will use tfds.load(dataset_name, ...) — it downloads on the user's machine when they run the notebook.
     c. If no suitable dataset exists: tell the user no public dataset was found, synthetic data will be used.
